...
export default withRouter(function Login(
  ...
  if (queries.error || queries.returnTo) {
    const { error, returnTo, username } = queries;
    const state = {
      ...(queries.error ? { error, username } : {}),
      ...(!!returnTo ? { returnTo } : {}),
    };
  ...
  const { returnTo, error, username, createdWithSuccess } =
        ...
        <Container className={styles.centralContainer} spacing="pt-10w">
          ...
          <Row justifyContent="right">
            <Col className={styles.resetFlexCol}>
              <Button size="md" onClick={onClickSubmitForm}>
                Se connecter
              </Button>
            </Col>
          </Row>
